...
export default function TVShowsPage() {
    ...
    useEffect(() => {
        fetch(`${API_BASE}/genres?type=TVSeries`)
            .then(res => res.json())
    ...
    useEffect(() => {
        ...
        if (ongoing) params.append('ongoing', ongoing)
        if (year) params.append('year', year)

        fetch(`${API_BASE}/tvshows?${params.toString()}`)
            .then(res => res.json())
            .then(data => {
    ...
    return (
        <main className="page">
                ...
                <div className="domain-filters">
                    <div className="filter-group">
                        <select 
                            ...
                            value={ongoing} 
                            onChange={(e) => handleParamChange('ongoing', e.target.value)}
                        >
                            <option value="">All Statuses</option>
                            <option value="true">Ongoing Series</option>
                            <option value="false">Ended Series</option>
                        </select>
                    </div>
                    <div className="filter-group">
                        <select 
            ...
            {loading ? (
                <p className="status">Loading TV shows...</p>
            ) : tvShows.length === 0 ? (
                <p className="status">No TV shows found.</p>
            ) : (
                <>
